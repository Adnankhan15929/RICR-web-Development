...
const Currency = () => {
  ...
  const convert = async () =>{
    ...
    try {
        const res = await axios.get(`https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@latest/v1/currencies/${from.split(" ")[0].toLowerCase()}.json`)
        ...
        <div className="border rounded shadow w-3xl p-3 mx-auto space-y-5 bg-h">
          <div className="relative grid grid-cols-2 gap-10">
                ...
                <div className="absolute left-1/2 -translate-x-[50%] top-3  ">
                    <button className="bg-li text-tt scale-120 hover:scale-170 hover:duration-300 duration-300 hover:bg-tt hover:text-white " 
                    onClick={swap}>
                        <LuArrowRightLeft />
                    </button>
                </div>
          ...
          <div className="flex gap-3 items-center">
            ...
            <input type="number" name="fromAmt" value={fromAmt} onChange={(e)=>setFromAmt(e.target.value)} className="w-full p-3 rounded border" placeholder="Enter the amount"/>
          </div>
          <button className="w-full px-4 py-2 rounded bg-tt text-white hover:bg-li hover:text-tt "
          onClick={convert}>
            Convert
          </button>
          ...
          <div className="flex gap-3 items-center pb-2">
            <label htmlFor="toAmt">Converted Amount: {toAmt? toAmt:"XXXXX"}</label>
          </div>
        </div>
      </div>
    </>
